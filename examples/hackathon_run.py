...
def run_exogenous_experiment(path_to_file, len_forecast=12,
    ...
    train_input, predict_input, task = prepare_input_data(len_forecast=len_forecast,
                                                          train_data_features=target,
                                                          train_data_target=target,
                                                          test_data_features=target)
    ...
    if with_visualisation:
        ...
        plt.plot(range(len(target), len(target) + len_forecast), predicted, label='Forecast')
        # Plot black line which divide our array into train and test
        plt.plot([len(target), len(target)],
